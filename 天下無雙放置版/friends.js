...
const Friends = {
    ...
    decode(code) {
        code = (code || '').trim();
        if (!code.startsWith('XIU:')) throw new Error(T('friend.badCode','不是有效的仙途分享码'));
        const obj = JSON.parse(this._ub64(code.slice(4)));
        if (!obj || !obj.type) throw new Error(T('friend.badContent','分享码内容有误'));
        return obj;
    },
    ...
    makeGift(p, gift) {
        ...
        if (gift.stone > 0) {
            if (p.stone < gift.stone) throw new Error(T('friend.noStoneGift','灵石不足，无法赠送'));
        ...
        if (gift.item && gift.item.count > 0) {
            ...
            if (have < count) throw new Error(T('friend.insufficientGift','赠品数量不足'));
        ...
        if (!payload.stone && !Object.keys(payload.pills).length && !Object.keys(payload.materials).length) {
            throw new Error(T('friend.pickGift','请先选择要赠送的灵石或物品'));
        }
        // 扣除己方
    ...
    import(p, code) {
        ...
        throw new Error(T('friend.unknownType','未知分享码类型'));
    },

    /* 描述一笔馈赠内容 */
    describeGift(g) {
        const parts = [];
        if (g.stone) parts.push(fmtNum(g.stone) + T('friend.stone','灵石'));
        Object.entries(g.pills || {}).forEach(([id, c]) => { const m = getPill(id); if (m) parts.push(DN(m) + '×' + c); });
        Object.entries(g.materials || {}).forEach(([id, c]) => { const m = getMaterial(id); if (m) parts.push(DN(m) + '×' + c); });
        return parts.join(T('friend.sep','、')) || T('friend.empty','（空）');
    }
};
