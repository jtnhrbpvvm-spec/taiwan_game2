...
const Cultivate = {
    ...
    async breakThrough(player) {
        ...
        if (player.realmLayer >= getRealm(player.realmIdx).layers) {
            if (player.realmIdx >= GameConfig.realms.length - 1) {
                if (typeof UI !== 'undefined') UI.toast(T('cult.maxRealm','已达最高境界，无法突破！'), 'bad');
                return false;
            }
            ...
            if (player.zhanli < cost) {
                if (typeof UI !== 'undefined') UI.toast(T('cult.zhanliLackBreak','战力不足，无法突破！'), 'bad');
                return false;
            }
            // 额外灵石门槛
            const stoneCost = this.getBreakthroughStoneCost(player);
            if (player.stone < stoneCost) {
                if (typeof UI !== 'undefined') UI.toast(T('cult.stoneLackPre','灵石不足，突破需')+fmtNum(stoneCost)+T('cult.lingStone','灵石'), 'bad');
                return false;
            }
            ...
            if (player.inventory.pill.p_breakthrough > 0) {
                ...
                if (typeof UI !== 'undefined') UI.toast(T('cult.useBreakPill','使用破障丹，成功率+20%'), 'gold');
            }
            if (btBonus > 0) {
            ...
            if (Math.random() < rate) {
                ...
                if (typeof Talent !== 'undefined') {
                    ...
                    if (typeof UI !== 'undefined') UI.toast(T('cult.talentAwaken','境界突破，觉醒天赋点 +3！前往「天赋」面板修习'), 'gold');
                }
                const newRealm = getRealm(player.realmIdx);
                if (typeof UI !== 'undefined') {
                    UI.showBreakthroughFX(T('cult.breakFx','突破！')+DN(newRealm)+T('cult.layer','一层'));
                    UI.toast(T('cult.congrats','恭喜突破至')+DN(newRealm)+T('cult.realm','境界！'), 'gold');
                    UI.addLog(T('cult.daoxin','道心通明，突破至')+DN(newRealm)+T('cult.layer','一层')+'！', 'evt');
                ...
                if (typeof UI !== 'undefined') {
                    UI.toast(T('cult.breakFail','突破失败，走火入魔！'), 'bad');
                    UI.addLog(T('cult.breakFailLoss','突破失败，损失')+fmtNum(loss)+T('cult.zhanliUnit','战力'), 'bad');
                }
                this.save(player);
                return false;
            }
        ...
        if (player.realmLayer % 5 === 0 && typeof Talent !== 'undefined') {
            ...
            if (typeof UI !== 'undefined') UI.toast(T('cult.layerProgress','境界精进，天赋点 +1'), 'gold');
        }
        const realm = getRealm(player.realmIdx);
        if (typeof UI !== 'undefined') {
            UI.showBreakthroughFX(DN(realm)+cnNum(player.realmLayer)+T('cult.layerUnit','层'));
            UI.toast(T('cult.breakTo','突破至')+DN(realm)+cnNum(player.realmLayer)+T('cult.layerUnit','层')+'（'+T('cult.lifeAdd','寿元+')+fmtNum(layerLife)+'）', 'gold');
            UI.addLog(T('cult.zhanliImprove','战力精进，突破至')+DN(realm)+cnNum(player.realmLayer)+T('cult.layerUnit','层'), 'evt');
    ...
    enlighten(player, type) {
        ...
        if (lv >= this.WUXING_MAX) {
            if (typeof UI !== 'undefined') UI.toast(T('cult.wuxingMax','悟性已臻化境，无法再顿悟'), 'bad');
            return false;
        }
        ...
        if (!ok) {
            if (typeof UI !== 'undefined') UI.toast(T('cult.enlightenLack','资源不足，无法顿悟'), 'bad');
            return false;
        }
        player.wuxingLevel = lv + 1;
        this.save(player);
        if (typeof UI !== 'undefined') {
            UI.toast(T('cult.enlightenOk','顿悟成功！悟性升至 ')+player.wuxingLevel+T('cult.zhong',' 重，每次修炼战力+')+Math.round((this.wuxingTapMult(player) - 1) * 100)+T('cult.rateMid','%、修炼速率+')+Math.round((this.wuxingRateMult(player) - 1) * 100)+'%', 'gold');
            UI.addLog(T('cult.lingtai','灵台清明，悟性突破至第 ')+player.wuxingLevel+T('cult.zhongEnd',' 重'), 'evt');
        }
        return true;
    },
    ...
    seclude(player, years) {
        years = Math.floor(years);
        if (!years || years <= 0) {
            if (typeof UI !== 'undefined') UI.toast(T('cult.chooseSeclude','请选择闭关时长'), 'bad');
            return null;
        }
        if (years > (player.lifespan || 0)) {
            if (typeof UI !== 'undefined') UI.toast(T('cult.lifeLack','寿元不足，仅余')+player.lifespan+T('cult.year','年'), 'bad');
            return null;
        }
        ...
        if (player.lifespan <= 0) {
            player.lifespan = 0;
            this.save(player);
            if (typeof UI !== 'undefined') UI.addLog(player.name+T('cult.yuhua','寿元已尽，羽化归虚……'), 'evt');
        ...
        if (typeof UI !== 'undefined') UI.addLog(T('cult.secludePre','闭关')+years+T('cult.yearGainPre','年，悟得')+fmtNum(xiuGain)+T('cult.zhanliCostLife','战力（耗寿元')+years+T('cult.yearEnd','年）'), 'evt');
    ...
    youli(player, years) {
        years = Math.floor(years);
        if (!years || years <= 0) {
            if (typeof UI !== 'undefined') UI.toast(T('cult.chooseYouli','请选择游历时长'), 'bad');
            return null;
        }
        if (years > (player.lifespan || 0)) {
            if (typeof UI !== 'undefined') UI.toast(T('cult.lifeLack','寿元不足，仅余')+player.lifespan+T('cult.year','年'), 'bad');
        ...
        if (Math.random() < 0.08) {
            ...
            fortuneText = T('cult.fortune','途中逢奇遇，额外获得 ')+fmtNum(bonus)+T('cult.lingRare',' 灵石及稀有宝物！');
        }
        ...
        if (player.lifespan <= 0) {
            ...
            this.save(player);
            if (typeof UI !== 'undefined') UI.addLog(player.name+T('cult.youliEnd','游历')+years+T('cult.yuhua','年，寿元已尽，羽化归虚……'), 'evt');
            if (typeof Game !== 'undefined' && typeof Game.onLifespanZero === 'function') {
                Game.onLifespanZero(player);
            }
        ...
        this.save(player);
        if (typeof UI !== 'undefined') UI.addLog(T('cult.youliPre','游历')+years+T('cult.yearGetPre','年，收获')+fmtNum(stoneGain)+T('cult.lingTreasure','灵石及诸多宝物'), 'evt');
        if (typeof Sound !== 'undefined') Sound.play('coin');
    ...
    exchangeZhanliForStone(player, xiuAmount) {
        ...
        if (!xiuAmount || xiuAmount <= 0) {
            if (typeof UI !== 'undefined') UI.toast(T('cult.inputZhanli','请输入兑换的战力数量'), 'bad');
            return null;
        }
        if (xiuAmount > (player.zhanli || 0)) {
            if (typeof UI !== 'undefined') UI.toast(T('cult.zhanliLack2','战力不足'), 'bad');
            return null;
        }
        const stone = Math.floor(xiuAmount * this.ZHANLI_TO_STONE_RATE);
        if (stone <= 0) {
            if (typeof UI !== 'undefined') UI.toast(T('cult.exchangeLittle','兑换所得灵石过少'), 'bad');
            return null;
        }
        ...
        this.save(player);
        if (typeof UI !== 'undefined') {
            UI.addLog(T('cult.exchangePre','以')+fmtNum(xiuAmount)+T('cult.exchangeMid','战力，兑换得')+fmtNum(stone)+T('cult.lingStone','灵石'), 'evt');
        }
    ...
    getRealmName(player) {
        const realm = getRealm(player.realmIdx);
        return DN(realm)+cnNum(player.realmLayer)+T('cult.layerUnit','层');
    },
    ...
    toggleCultivate() {
        ...
        if (btn) btn.textContent = Game.cultivating ? T('cult.pauseCult','暂停修炼') : T('cult.startCult','开始修炼');
        if (status) status.textContent = Game.cultivating ? T('cult.qiji','气机流转，战力缓缓增长') : T('cult.paused','修炼已暂停');
        if (typeof UI !== 'undefined') UI.toast(Game.cultivating ? T('cult.startCult','开始修炼') : T('cult.paused','修炼已暂停'), 'good');
    },
    ...
    resolveTribulation(player, mode) {
        ...
        if (mode === 'protect') {
            ...
            if ((player.stone || 0) < cost) {
                return { mode, fail: true, msg: T('cult.protectLack','灵石不足，无法请护道人化解天劫') };
            }
            player.stone -= cost;
            ...
            return { mode, success: true, protected: true, bonus: pb,
                msg: T('cult.protectResolvePre','请护道人耗')+fmtNum(cost)+T('cult.lingResolve','灵石化解')+DN(trib)+T('cult.safePass','，平安渡过（修炼速率+')+Math.round(pb.xiuMult*100)+T('cult.rateGetMid','%、灵石获取+')+Math.round(pb.stoneMult*100)+T('cult.pctEnd','%）') };
        }
        // 硬抗
        const chance = this.calcTribSuccess(player, trib);
        if (Math.random() < chance) {
            ...
            return { mode, success: true, bonus: sb,
                msg: T('cult.hardWinPre','硬抗')+DN(trib)+T('cult.hardWinMid','成功！劫后余韵加身：修炼速率+')+Math.round(sb.xiuMult*100)+T('cult.rateGetMid','%、灵石获取+')+Math.round(sb.stoneMult*100)+T('cult.pctEnd','%') };
        }
        ...
        return { mode, success: false, penalty: fp, xiuLost: loss,
            msg: T('cult.hardFailPre','硬抗')+DN(trib)+T('cult.hardFailMid','失败！境界回落、损失')+fmtNum(loss)+T('cult.zhanliLossMid','战力、寿元-')+fp.lifeLoss };
    },
    ...
    reincarnate(player, mode) {
        ...
        if ((player.rebirth || 0) >= (cfg.maxRebirth || 100)) {
            if (typeof UI !== 'undefined') UI.toast(T('cult.rebirthCapPre','已达轮回上限 ')+(cfg.maxRebirth || 100)+T('cult.rebirthCapEnd',' 世，不可再轮回'), 'bad');
        ...
        if (mode === 'pill') {
            ...
            if (have < cfg.herbCost) {
                if (typeof UI !== 'undefined') UI.toast(T('cult.needHerbPre','需 ')+cfg.herbCost+T('cult.zhu',' 株')+DN(getMaterial(cfg.herbId))+T('cult.shopSale','（坊市有售）'), 'bad');
            ...
            if ((player.zhanli || 0) < cap) {
                if (typeof UI !== 'undefined') UI.toast(T('cult.needZhanliPre','需先达战力 ')+fmtNum(cap)+T('cult.freeRebirthEnd','（战力上线，随轮回次数递增）方可免费轮回'), 'bad');
        ...
        if (typeof UI !== 'undefined') {
            ...
            UI.toast(usedPill
                ? T('cult.reincarnatePillPre','转世第 ')+player.rebirth+T('cult.shiPill',' 世！根基大进：气血 ×')+rb.hpMult.toFixed(2)+T('cult.hpAtkMid',' · 攻击 ×')+rb.atkMult.toFixed(2)+T('cult.xiuGainEnd',' · 修炼收益 +')+xiuPct+'%'
                : T('cult.reincarnateFreePre','已免费轮回至第 ')+player.rebirth+T('cult.shiFree',' 世，重立道基（生命/攻击加成与修炼收益 +')+xiuPct+T('cult.pctPerm','% 永久保留）'), 'gold');
            UI.addLog(usedPill ? T('cult.pillLogPre','历经')+player.rebirth+T('cult.pillLogEnd','世轮回，道基愈发浑厚') : player.name+T('cult.sangong','散功重修，再踏仙途（第 ')+player.rebirth+T('cult.shiEnd',' 世）'), 'evt');
            UI.renderAll();
