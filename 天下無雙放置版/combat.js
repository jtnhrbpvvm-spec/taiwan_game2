...
const Combat = {
    ...
    startBattleWithEnemy(player, enemyTpl, isHard = false) {
        ...
        this.state = {
            player: {
                name: player.name,
                avatar: [T('combat.avatarDao','道'), T('combat.avatarXian','仙'), T('combat.avatarMo','魔'), T('combat.avatarFo','佛')][player.avatar],
                elem: player.element,
                hp: effHp,
                maxHp: effHp,
        ...
        this.addLog(T('combat.start','战斗开始！'), '');
        // 速度高者先手
    ...
    useSkill(skillId) {
        ...
        if ((st.skillCd[skillId] || 0) > 0) {
            if (typeof UI !== 'undefined') UI.toast(DN(skill) + T('combat.cd','冷却中'), 'bad');
            return;
        }
        // 灵力检查
        ...
        if (skill.healMult) {
            ...
            this.addLog(T('combat.cast','施展') + DN(skill) + T('combat.restoreHp','，恢复') + heal + T('combat.hpUnit','气血'), 'heal');
            this.showFloat('player', heal, 'heal');
        } else if (skill.shield) {
            // 护盾技能
            const shield = Math.floor(st.maxHp * skill.shield * lvlMult);
            st.shield = shield;
            this.addLog(T('combat.cast','施展') + DN(skill) + T('combat.gainShield','，获得') + shield + T('combat.shieldUnit','护盾'), 'heal');
        } else if (skill.buff) {
            // 增益buff
            st.buffs.push({ ...skill.buff, lvlMult });
            this.addLog(T('combat.cast','施展') + DN(skill) + T('combat.comma','，') + skill.buff.type + '+' + Math.floor(skill.buff.value * lvlMult), 'heal');
        } else {
            // 攻击技能
    ...
    useFabao() {
        ...
        if (fs.shield) {
            ...
            this.addLog(T('combat.activateFabao','激发法宝') + DN(player.fabao) + T('combat.colon','：') + DN(fs) + T('combat.gainShield','，获得') + T('combat.shieldUnit','护盾'), 'heal');
        } else if (fs.reflect) {
            player.reflect = fs.reflect;
            player.reflectTurns = fs.turns || 2;
            this.addLog(T('combat.activateFabao','激发法宝') + DN(player.fabao) + T('combat.colon','：') + DN(fs) + T('combat.reflectDmg','，反弹伤害'), 'heal');
        } else if (fs.dmgMult) {
            // 法宝攻击
            ...
            this.addLog(T('combat.activateFabao','激发法宝') + DN(player.fabao) + T('combat.colon','：') + DN(fs) + T('combat.dealDmg','，造成') + dmg + T('combat.dmgUnit','伤害') + (elemMult > 1 ? T('combat.elemCtr','（五行克制）') : ''), 'dmg');
    ...
    dealDamage(from, to, skill, lvlMult = 1, isCombo = false) {
        ...
        if (defender.shield > 0) {
            ...
            dmg -= absorbed;
            if (absorbed > 0) this.addLog(DN(defender) + T('combat.shieldAbsorb','护盾抵消') + absorbed + T('combat.dmgUnit','伤害'), 'heal');
        }
        if (dmg > 0) {
            defender.hp -= dmg;
            // 反弹伤害
            if (defender.reflect > 0) {
                ...
                this.addLog(DN(defender) + T('combat.reflect','反弹') + reflectDmg + T('combat.dmgUnit','伤害'), 'dmg');
                this.showFloat(from, reflectDmg, 'dmg');
            }
        }

        const logType = isCrit ? 'crit' : (elemMult > 1.3 ? 'elem' : 'dmg');
        let logMsg = DN(attacker) + T('combat.cast','施展') + (skill.name ? DN(skill) : T('combat.attack','攻击')) + T('combat.dealDmg','，造成') + dmg + T('combat.dmgUnit','伤害');
        if (isCrit) logMsg += T('combat.crit','（暴击）');
        if (elemMult > 1.3) logMsg += T('combat.elemCtr','（五行克制）');
        if (isCombo) logMsg += T('combat.comboHit','（连击）');
        ...
        if (skill.debuff && dmg > 0) {
            ...
            this.addLog(DN(defender) + T('combat.affBy','被') + skill.debuff.type + T('combat.affEnd','影响'), 'elem');
        }
    },
    ...
    petAction() {
        ...
        if (petSkill.shield) {
            ...
            this.addLog(DN(player.pet) + T('combat.cast','施展') + DN(petSkill) + T('combat.protectShield','，护主加盾'), 'heal');
            ...
            this.state.enemy.hp -= dmg;
            this.addLog(DN(player.pet) + T('combat.cast','施展') + DN(petSkill) + T('combat.dealDmg','，造成') + dmg + T('combat.dmgUnit','伤害'), 'dmg');
    ...
    enemyTurn() {
        ...
        if (enemy.sealTurns > 0) {
            enemy.sealTurns--;
            this.addLog(DN(enemy) + T('combat.sealed','被封印，无法行动'), 'miss');
            this.endTurn();
            return;
        }
        ...
        if (dmg > 0) {
            ...
            if (player.reflect > 0) {
                ...
                enemy.hp -= reflectDmg;
                this.addLog(T('combat.reflectDmg2','反弹') + reflectDmg + T('combat.dmgUnit','伤害'), 'dmg');
                this.showFloat('enemy', reflectDmg, 'dmg');
            }
        }
        const logType = isCrit ? 'crit' : (elemMult > 1.3 ? 'elem' : 'dmg');
        let logMsg = DN(enemy) + (isStrong ? T('combat.strong','强力') : '') + T('combat.attack2','攻击') + T('combat.dealDmg','，造成') + dmg + T('combat.dmgUnit','伤害');
        if (isCrit) logMsg += T('combat.crit','（暴击）');
        if (elemMult > 1.3) logMsg += T('combat.elemCtr','（五行克制）');
        this.addLog(logMsg, logType);
    ...
    checkEnd() {
        ...
        if (this.state.player.hp <= 0) {
            ...
            this.addLog(T('combat.lose','你战败了...'), 'miss');
        ...
        if (this.state.enemy.hp <= 0) {
            ...
            this.addLog(T('combat.win','你战胜了对手！'), 'heal');
    ...
    endBattle(victory) {
        ...
        if (victory) {
            ...
            if (typeof UI !== 'undefined') {
                UI.hideBattle();
                UI.toast(T('combat.winToast','胜利！获得') + fmtNum(zhanliReward) + T('combat.zhanli','战力') + T('combat.comma','，') + fmtNum(stoneReward) + T('combat.stone','灵石'), 'gold');
                UI.addLog(T('combat.defeat','战胜') + DN(enemyTpl) + T('combat.gain','，获得') + fmtNum(zhanliReward) + T('combat.zhanli','战力'), 'evt');
                drops.forEach(d => {
                    const tpl = getEquipTemplate(d.id) || getPill(d.id) || getMaterial(d.id);
                    UI.addLog(T('combat.dropGot','获得掉落：') + DN(tpl) + '×' + d.count, 'evt');
                    UI.toast(T('combat.got','获得：') + DN(tpl) + '×' + d.count, 'gold');
                });
                UI.renderAll();
            }
            // 任务进度
            ...
            if (typeof UI !== 'undefined') {
                UI.hideBattle();
                UI.toast(T('combat.defeatedLoss','战败，损失') + fmtNum(loss) + T('combat.zhanli','战力'), 'bad');
                UI.addLog(T('combat.lostTo','败于') + DN(enemyTpl) + T('combat.lost','，损失') + fmtNum(loss) + T('combat.zhanli','战力'), 'bad');
                UI.renderAll();
            }
            SaveSystem.save(player);
        }
    ...
    flee() {
        ...
        if (Math.random() < 0.5) {
            this.state.finished = true;
            this.addLog(T('combat.fleeOk','成功逃遁'), 'miss');
            if (typeof UI !== 'undefined') {
                UI.hideBattle();
                UI.toast(T('combat.escapeOk','成功逃脱'), 'good');
            }
            this.state = null;
        } else {
            this.addLog(T('combat.fleeFail','逃遁失败！'), 'bad');
            setTimeout(() => this.enemyTurn(), 800);
        }
    },
