...
const Inventory = {
    ...
    equip(player, uid) {
        ...
        if (typeof UI !== 'undefined') UI.toast(T('inv.equipped','已装备：') + DN(tpl), 'good');
        if (typeof UI !== 'undefined') UI.addLog(T('inv.equipLog','装备 ') + DN(tpl));
        this.save(player);
        return true;
    },
    ...
    unequip(player, slot) {
        ...
        if (typeof UI !== 'undefined') UI.toast(T('inv.unequipped','已卸下：') + DN(tpl), 'good');
        this.save(player);
        return true;
    },
    ...
    enhance(player, uid) {
        ...
        if (curEnhance >= 100) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.enhMax','已达强化上限+100！'), 'bad');
            return false;
        }
        // 强化消耗战力（随等级递增）
        ...
        if (player.zhanli < cost) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.noZhanli','战力不足！需要') + cost + T('combat.zhanli','战力'), 'bad');
            return false;
        }
        player.zhanli -= cost;
        eqInstance.enhance = curEnhance + 1;
        if (typeof UI !== 'undefined') {
            UI.toast(T('inv.enhOk','强化成功！+') + (curEnhance + 1) + T('inv.enhCost','（耗') + cost + T('inv.enhCostEnd','战力）'), 'gold');
            UI.addLog(DN(tpl) + T('inv.enhTo','强化至+') + (curEnhance + 1) + T('inv.enhUse','，耗') + cost + T('combat.zhanli','战力'), 'evt');
        }
        this.save(player);
        return true;
    },
    ...
    decompose(player, uid) {
        ...
        if (tpl.quality >= 1) {
            ...
            if (typeof UI !== 'undefined') UI.toast(T('inv.decompGain','分解获得') + total + T('combat.stone','灵石') + ' + ' + DN(getMaterial(matId)) + '×' + tpl.quality, 'gold');
        } else {
            if (typeof UI !== 'undefined') UI.toast(T('inv.decompGain','分解获得') + total + T('combat.stone','灵石'), 'gold');
        }
        this.save(player);
        return true;
    },
    ...
    usePill(player, pillId) {
        ...
        switch (eff.type) {
            ...
            case 'perm_all':
                ...
                msg = T('inv.permAll','永久+') + eff.value + T('inv.permAllAtk','攻击 +') + (eff.value * 5) + T('combat.hpUnit','气血');
                break;
            case 'buff_atk':
                player.tempBuffAtk = { value: eff.value, turns: eff.turns };
                msg = T('inv.buffAtk','战斗中攻击+') + eff.value + T('inv.turns','（') + eff.turns + T('inv.turnsEnd','回合）');
                break;
            case 'buff_def':
                player.tempBuffDef = { value: eff.value, turns: eff.turns };
                msg = T('inv.buffDef','战斗中防御+') + eff.value + T('inv.turns','（') + eff.turns + T('inv.turnsEnd','回合）');
                break;
            case 'breakBonus':
                // 立即触发突破，使用此丹药时自动调用突破
                msg = T('inv.breakPill','破障丹已使用，下次突破成功率+20%');
                player.breakBonus = (player.breakBonus || 0) + eff.value;
                break;
            case 'perm_def':
                player.permBonus.def = (player.permBonus.def || 0) + eff.value;
                msg = T('inv.permDef','永久防御+') + eff.value;
                break;
            case 'perm_crit':
                player.permBonus.crit = (player.permBonus.crit || 0) + eff.value;
                msg = T('inv.permCrit','永久暴击率+') + (eff.value * 100).toFixed(0) + '%';
                break;
            case 'perm_spd':
                player.permBonus.spd = (player.permBonus.spd || 0) + eff.value;
                msg = T('inv.permSpd','永久速度+') + eff.value;
                break;
            case 'stone':
                ...
                msg = T('inv.gainZhanli','获得') + _stGain + T('combat.stone','灵石');
                break;
        }
        if (typeof UI !== 'undefined') {
            UI.toast(T('inv.use','使用') + DN(pill) + T('combat.colon','：') + msg, 'good');
            UI.addLog(T('inv.use','使用') + DN(pill) + T('combat.comma','，') + msg);
        }
        this.save(player);
        return true;
    },
    ...
    craftPill(player, pillId) {
        ...
        for (const matId in pill.craft.materials) {
            ...
            if (have < need) {
                if (typeof UI !== 'undefined') UI.toast(T('inv.matLack','材料不足：') + DN(getMaterial(matId)) + '×' + (need - have), 'bad');
                return false;
            }
        }
        // 检查灵石
        if (player.stone < pill.craft.cost) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.noStone','灵石不足！需要') + pill.craft.cost + T('combat.stone','灵石'), 'bad');
            return false;
        }
        // 扣除材料
        for (const matId in pill.craft.materials) {
        ...
        if (typeof UI !== 'undefined') {
            UI.toast(big ? (T('inv.alchemyBig','炼丹大成功！') + DN(pill) + '×' + count) : (T('inv.alchemyOk','炼制成功：') + DN(pill) + '×1'), big ? 'gold' : 'gold');
            UI.addLog(big ? (T('inv.alchemyBigLog','炼丹大成功，') + DN(pill) + '×' + count) : (T('inv.alchemyLog','炼制丹药 ') + DN(pill)), 'evt');
        }
        this.save(player);
        return true;
    },
    ...
    craftEquip(player, eqId) {
        ...
        for (const mid in tpl.craft.materials) {
            ...
            if (have < need) {
                if (typeof UI !== 'undefined') UI.toast(T('inv.matLack','材料不足：') + DN(getMaterial(mid)) + '×' + (need - have), 'bad');
                return false;
            }
        }
        // 检查灵石
        if (player.stone < tpl.craft.cost) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.noStone','灵石不足！需要') + tpl.craft.cost + T('combat.stone','灵石'), 'bad');
            return false;
        }
        // 扣除材料与灵石
        for (const mid in tpl.craft.materials) {
        ...
        if (typeof UI !== 'undefined') {
            UI.toast(T('inv.forge','锻造') + (tier >= 3 ? T('inv.topQuality','极品') : T('inv.success','成功')) + T('combat.colon','：') + DN(tpl) + T('inv.bracket','【') + DN(q) + T('inv.bracketEnd','】'), 'gold');
            UI.addLog(T('inv.forgeLog','于器炉锻造 ') + DN(tpl) + T('inv.paren','（') + DN(q) + T('inv.parenEnd','）'), 'evt');
        }
        this.save(player);
        return true;
    },
    ...
    learnGongfa(player, gfId) {
        if (player.inventory.gongfa.includes(gfId)) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.gfOwned','已拥有此功法'), 'bad');
            return false;
        }
        const gf = getGongfa(gfId);
        if (!gf) return false;
        if (player.realmIdx < gf.realmReq) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.realmLow','境界不足，需') + DN(getRealm(gf.realmReq)) + T('shop.realm','境界'), 'bad');
            return false;
        }
        player.inventory.gongfa.push(gfId);
        if (typeof UI !== 'undefined') UI.toast(T('inv.learnGf','习得功法：') + DN(gf), 'gold');
        this.save(player);
        return true;
    },
    ...
    equipGongfa(player, gfId) {
        if (!player.inventory.gongfa.includes(gfId)) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.noGf','尚未习得此功法'), 'bad');
            return false;
        }
        player.gongfa = gfId;
        const gf = getGongfa(gfId);
        if (typeof UI !== 'undefined') {
            UI.toast(T('inv.equipGf','已装备功法：') + DN(gf), 'good');
            UI.addLog(T('inv.changeGf','改修 ') + DN(gf));
        }
        this.save(player);
        return true;
    },
    ...
    unequipGongfa(player) {
        if (!player.gongfa) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.noGfEquipped','当前未装备任何功法'), 'bad');
            return false;
        }
        const gf = getGongfa(player.gongfa);
        player.gongfa = null;
        if (typeof UI !== 'undefined') {
            UI.toast(T('inv.unequipGf','已卸下功法：') + (gf ? DN(gf) : ''), 'good');
            UI.addLog(T('inv.unequipGfLog','卸下功法 ') + (gf ? DN(gf) : ''));
        }
        this.save(player);
        return true;
    },
    ...
    catchPet(player, petId) {
        ...
        if (player.realmIdx < pet.realmReq) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.realmLowPet','境界不足，无法收服') + DN(pet), 'bad');
            return false;
        }
        if (player.pets && player.pets.find(x => x.id === petId)) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.petOwned','已收服此灵宠'), 'bad');
            return false;
        }
        ...
        if (typeof UI !== 'undefined') {
            UI.toast(T('inv.catchPet','收服灵宠：') + DN(pet) + T('inv.excl','！'), 'gold');
            UI.addLog(T('inv.catchPetLog','收服灵宠 ') + DN(pet) + T('inv.excl','！'), 'evt');
        }
        this.save(player);
        return true;
    },
    ...
    learnSkill(player, skillId) {
        if (player.skills[skillId]) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.skillLearned','已学习此技能'), 'bad');
            return false;
        }
        const skill = getSkill(skillId);
        if (!skill) return false;
        if (player.realmIdx < skill.realmReq) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.realmLow','境界不足，需') + DN(getRealm(skill.realmReq)) + T('shop.realm','境界'), 'bad');
            return false;
        }
        if (player.zhanli < skill.learnCost) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.noZhanli','战力不足！需要') + fmtNum(skill.learnCost) + T('combat.zhanli','战力'), 'bad');
            return false;
        }
        player.zhanli -= skill.learnCost;
        player.skills[skillId] = 1;
        if (typeof UI !== 'undefined') {
            UI.toast(T('inv.learnSkill','习得神通：') + DN(skill) + T('inv.excl','！'), 'gold');
            UI.addLog(T('inv.learnSkillLog','习得神通 ') + DN(skill), 'evt');
        }
        this.save(player);
        return true;
    },
    ...
    upgradeSkill(player, skillId) {
        ...
        if (curLv >= 100) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.skillMax','技能已满级'), 'bad');
            return false;
        }
        const cost = Math.floor(skill.learnCost * Math.pow(skill.upCostMult || 1.2, curLv));
        if (player.zhanli < cost) {
            if (typeof UI !== 'undefined') UI.toast(T('inv.noZhanli','战力不足！需要') + fmtNum(cost) + T('combat.zhanli','战力'), 'bad');
            return false;
        }
        player.zhanli -= cost;
        player.skills[skillId] = curLv + 1;
        if (typeof UI !== 'undefined') {
            UI.toast(DN(skill) + T('inv.upTo','升至') + (curLv + 1) + T('inv.level','级'), 'gold');
        }
        this.save(player);
        return true;
    },
    ...
    sellItem(player, category, itemId, count = 1) {
        switch (category) {
            case 'pill': {
                ...
                player.stone += price;
                if (typeof UI !== 'undefined') UI.toast(T('inv.sell','出售') + DN(pill) + '×' + count + T('combat.comma','，') + T('inv.gain','获得') + price + T('combat.stone','灵石'), 'gold');
                break;
            }
            case 'material': {
                ...
                player.stone += price;
                if (typeof UI !== 'undefined') UI.toast(T('inv.sell','出售') + DN(mat) + '×' + count + T('combat.comma','，') + T('inv.gain','获得') + price + T('combat.stone','灵石'), 'gold');
                break;
            }
            case 'equipment': {
                // 装备出售需要按uid
    ...
    sellEquipment(player, uid) {
        ...
        player.inventory.fabao = player.inventory.fabao.filter(e => e.uid !== uid);
        player.stone += price;
        if (typeof UI !== 'undefined') UI.toast(T('inv.sell','出售') + DN(tpl) + T('combat.comma','，') + T('inv.gain','获得') + price + T('combat.stone','灵石'), 'gold');
        this.save(player);
        return true;
    },
