...
const SaveSystem = {
    ...
    _safeStringify(obj) {
        try {
            ...
            try {
                ...
                console.error(T('save.serializeFatal','序列化彻底失败:'), e2);
                return null;
            }
        }
    },
    ...
    save(player, slotId) {
        ...
        if (!slotId) {
            ...
            console.error(T('save.noSlot','存档失败：未指定存档槽'));
            return false;
        }
        try {
            player.lastSave = Date.now();
            ...
            if (!data) {
                ...
                console.error(T('save.serializeFail','存档跳过：序列化失败，保留旧档'));
                return false;
            }
            ...
            console.error(T('save.saveFail','存档失败:'), e);
            return false;
        }
    },
    ...
    load(slotId) {
        ...
        try {
            ...
            console.error(T('save.loadFail','读档失败:'), e);
            return null;
        }
    },
    ...
    reset() {
        try {
            ...
            console.error(T('save.resetFail','重置失败:'), e);
            return false;
        }
    },
    ...
    */
    async exportSave(player, password) {
        try {
            ...
            console.error(T('save.exportFail','导出失败:'), e);
            return '';
        }
    },
    ...
    */
    ...
    metaFromPlayer(p) {
        return {
            name: p.name || T('save.unnamed','无名修士'),
            avatar: p.avatar || 0,
        ...
        try {
            ...
            if (!idx.slots['1']) {
                ...
                idx.slots['1'] = p
                    ...
                    : { name: T('save.unnamed','无名修士'), avatar: 0, element: 'metal', realmIdx: 0, realmLayer: 1, zhanli: 0, lifespan: 100, lastSave: Date.now() };
                idx.current = 1;
                this._writeIndex(idx);
            ...
            console.error(T('save.migrateFail','旧档迁移失败:'), e);
        }
    }
};
        ...
        this.cultivateTimer = setInterval(() => {
            ...
            if (_lifeCap < ZHANLI_CAP && (this.player.zhanli || 0) >= _lifeCap && this._capHintAt !== _lifeCap) {
                ...
                if (typeof UI !== 'undefined') {
                    UI.toast(T('save.capReached','战力已达今生上线，修行入账停滞——轮回后方可继续增长！'), 'gold');
                    UI.addLog(T('save.capLog','今生战力已至上线，前往「转世轮回」重立道基'), 'evt');
                }
            }
            // UI刷新（节流：每秒更新一次）
