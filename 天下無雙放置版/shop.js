...
const Shop = {
    ...
    buy(player, type, id) {
        ...
        switch (type) {
            ...
            case 'fabao': {
                ...
                if (player.realmIdx < (tpl.quality - 1) && tpl.quality > 1) {
                    canBuy = false; reason = T('shop.realmLow','境界不足');
                }
                // 高阶法宝/装备：按 realmReq 精确解锁（大成期以上专属重宝）
                if (tpl.realmReq && player.realmIdx < tpl.realmReq) {
                    canBuy = false; reason = T('shop.needRealm','需') + DN(getRealm(tpl.realmReq)) + T('shop.realm','境界');
                }
                break;
            }
            case 'pill': {
            ...
            case 'gongfa': {
                ...
                if (player.inventory.gongfa.includes(id)) {
                    canBuy = false; reason = T('shop.owned','已拥有');
                }
        ...
        if (player.stone < price) {
            if (typeof UI !== 'undefined') UI.toast(T('shop.noStone','灵石不足，需要') + price + T('combat.stone','灵石'), 'bad');
            return;
        }
        player.stone -= price;
        ...
        if (typeof UI !== 'undefined') {
            let name = '';
            if (type === 'equipment' || type === 'fabao') name = DN(getEquipTemplate(id));
            else if (type === 'pill') name = DN(getPill(id));
            else if (type === 'material') name = DN(getMaterial(id));
            else if (type === 'gongfa') name = DN(getGongfa(id));
            else if (type === 'pet') name = DN(getPet(id));
            UI.toast(T('shop.buyOk','购买成功：') + name, 'gold');
            UI.addLog(T('shop.buyLog','坊市购买 ') + name + T('combat.comma','，') + T('shop.spend','花费') + price + T('combat.stone','灵石'), 'evt');
            UI.renderAll();
        }
    ...
    getCraftList(player) {
        return GameConfig.pills.filter(p => p.craft).map(p => {
            ...
            const mats = Object.entries(p.craft.materials).map(([mid, cnt]) => {
                ...
                return { name: DN(m), need: cnt, have, enough: have >= cnt };
            });
