...
const Explore = {
    ...
    enterScene(sceneId) {
        ...
        if (player.realmIdx < scene.realmReq) {
            if (typeof UI !== 'undefined') UI.toast(T('expl.realmLow','境界不足，需') + DN(getRealm(scene.realmReq)) + T('expl.realmLow2','境界'), 'bad');
            return;
        }
        // 探索冷却：防止同一场景反复触发白嫖战力/资源
        if (this.isOnCd()) {
            const remain = Math.ceil((this.cdUntil - Date.now()) / 1000);
            if (typeof UI !== 'undefined') UI.toast(T('expl.needRest','需静修片刻，方可再探（约') + remain + T('expl.sec','s）'), 'bad');
            return;
        }
        this.currentScene = sceneId;
        if (typeof UI !== 'undefined') {
            UI.toast(T('expl.enter','进入') + DN(scene), 'good');
            UI.addLog(T('expl.enterLog','进入 ') + DN(scene));
        }
        // 触发一次探索
        this.explore();
    },
    ...
    explore() {
        if (!this.currentScene) {
            if (typeof UI !== 'undefined') UI.toast(T('expl.pickScene','请先选择场景'), 'bad');
            return;
        }
    ...
    handleChoice(eventId, choiceIdx) {
        ...
        switch (choice.type) {
            case 'zhanli':
                ...
                if (typeof UI !== 'undefined') {
                    UI.toast(T('expl.gain','获得') + fmtNum(_incV) + T('expl.zhanli','战力'), 'gold');
                    UI.addLog(T('expl.luckyLog','奇遇：获得') + fmtNum(choice.value) + T('expl.zhanli','战力'), 'evt');
                }
            ...
            case 'item':
                ...
                if (typeof UI !== 'undefined') {
                    UI.toast(T('expl.gainItem','获得：') + DN(itemTpl) + '×' + (choice.count || 1), 'gold');
                    UI.addLog(T('expl.gainItemLog','获得 ') + DN(itemTpl) + '×' + (choice.count || 1), 'evt');
                }
                break;
            ...
            case 'quest': {
                if (typeof UI !== 'undefined') UI.toast(T('expl.newQuest','接到新的宗门任务'), 'good');
                ...
                if (typeof UI !== 'undefined') UI.addLog(T('expl.questPreLog','接到宗门任务，预付') + _qPre + T('expl.zhanli','战力'), 'evt');
                break;
            }
            ...
            case 'karma_good':
                ...
                if (Math.random() < 0.3) {
                    ...
                    if (typeof UI !== 'undefined') {
                        UI.toast(T('expl.goodKarma','善有善报，获得') + fmtNum(reward) + T('expl.zhanli','战力'), 'gold');
                        UI.addLog(T('expl.goodKarmaLog','行善积德，福报降临'), 'evt');
                    }
                } else {
                    if (typeof UI !== 'undefined') UI.toast(T('expl.beggarLeave','送人离去，心安理得'), 'good');
                }
                break;
            ...
            case 'karma_bad':
                ...
                if (typeof UI !== 'undefined') {
                    UI.toast(T('expl.robStone','抢得') + stoneGain + T('expl.stone','灵石'), 'gold');
                    UI.addLog(T('expl.robLog','抢夺他人，获得') + stoneGain + T('expl.stone','灵石'), 'bad');
                }
                // 30%概率触发复仇战斗
                if (Math.random() < 0.3) {
            ...
            case 'beggar_pay':
                if (player.stone < choice.cost) {
                    if (typeof UI !== 'undefined') UI.toast(T('expl.noStone','灵石不足'), 'bad');
                } else {
                    ...
                    if (Math.random() < 0.7) {
                        if (Math.random() < 0.3) {
                            ...
                            if (typeof UI !== 'undefined') {
                                UI.toast(T('expl.beggarTeach','乞丐传授你五行诀！'), 'gold');
                                UI.addLog(T('expl.beggarTeachLog','乞丐传授五行诀'), 'evt');
                            }
                        } else {
                            Inventory.addItem(player, 'p_xiu_pill', 2);
                            if (typeof UI !== 'undefined') UI.toast(T('expl.beggarPill','乞丐赠你筑基丹×2'), 'gold');
                        }
                    } else {
                        if (typeof UI !== 'undefined') UI.toast(T('expl.beggarGone','乞丐扬长而去'), 'good');
                    }
                }
                break;
            ...
            case 'hp_loss':
                ...
                if (typeof UI !== 'undefined') {
                    UI.toast(T('expl.loseHp','损失') + loss + T('expl.hp','气血'), 'bad');
                    UI.addLog(T('expl.hurtLog','意外受伤，损失') + loss + T('expl.hp','气血'), 'bad');
                }
                break;
            ...
            case 'dragon_tribute':
                ...
                if (player.stone >= 1000) {
                    ...
                    if (Math.random() < 0.5) {
                        ...
                        if (typeof UI !== 'undefined') UI.toast(T('expl.dragonPearl','龙王赏你灵珠×2'), 'gold');
                    } else {
                        // 直接收服幼龙（已扣灵石）
                        ...
                        if (pet && !hasDragon) {
                            ...
                            if (typeof UI !== 'undefined') {
                                UI.toast(T('expl.catchPet','收服灵宠：') + DN(pet) + T('expl.excl','！'), 'gold');
                                UI.addLog(T('expl.dragonAllow','龙王许你收服') + DN(pet) + T('expl.excl','！'), 'evt');
                            }
                        } else {
                            // 已有灵宠，给灵珠补偿
                            Inventory.addItem(player, 'm_pearl', 3);
                            if (typeof UI !== 'undefined') UI.toast(T('expl.havePet','你已有灵宠，龙王赐你灵珠×3'), 'gold');
                        }
                    }
                } else {
                    if (typeof UI !== 'undefined') UI.toast(T('expl.noStoneDragon','灵石不足，龙王不悦'), 'bad');
                }
                break;

            case 'nothing':
            default:
                if (typeof UI !== 'undefined') UI.toast(T('expl.nothing','无事发生'), 'good');
                break;
        }
    ...
    openTreasure(player, type) {
        ...
        if (type === 'treasure') {
            ...
            if (typeof UI !== 'undefined') UI.toast(T('expl.gain','获得') + stoneReward + T('expl.stone','灵石'), 'gold');
            if (Math.random() < 0.3) {
                // 30%概率只有灵石，无装备
        ...
        if (eqPool.length > 0) {
            ...
            if (typeof UI !== 'undefined') {
                UI.toast(T('expl.gainItem','获得：') + DN(eq) + T('expl.excl','！'), 'gold');
                UI.addLog(T('expl.chestLog','宝箱奇遇：获得') + DN(eq), 'evt');
            }
        }
        // 概率获得丹药
        if (Math.random() < 0.4) {
            ...
            if (pillPool.length > 0) {
                ...
                if (typeof UI !== 'undefined') UI.toast(T('expl.gainItem','获得：') + DN(pill) + '×1', 'gold');
            }
        }
    },
    ...
    triggerAuction(player) {
        ...
        if (typeof UI !== 'undefined') {
            UI.showModal({
                title: T('expl.auctionTitle','拍卖盛会'),
                body: `<p>${T('expl.auctionItem','拍卖品：')}<span style="color:${q.color}">${DN(item)}</span></p>
                       <p style="color:#9a8e7a;margin:8px 0">${DD(item)}</p>
                       <p>${T('expl.startPrice','起拍价：')}<span style="color:#d4af37">${price}${T('expl.stone','灵石')}</span></p>
                       <p>${T('expl.curStone','当前灵石：')}${fmtNum(player.stone)}</p>`,
                footer: [
                    { text: T('expl.bid','竞拍'), type: 'primary', action: () => {
                        if (player.stone >= price) {
                            player.stone -= price;
                            Inventory.addItem(player, item.id, 1);
                            if (typeof UI !== 'undefined') {
                                UI.toast(T('expl.bidOk','竞拍成功：') + DN(item), 'gold');
                                UI.addLog(T('expl.bidLog','拍卖获得 ') + DN(item), 'evt');
                                UI.hideModal();
                                UI.renderAll();
                            }
                            SaveSystem.save(player);
                    ...
                    { text: T('expl.giveup','放弃'), action: () => { if (typeof UI !== 'undefined') UI.hideModal(); } }
                ]
            });
        }
    },
    ...
    triggerRandomShop(player) {
        ...
        if (typeof UI !== 'undefined') {
            const body = random.map((r, i) => {
                let name = '';
                if (r.type === 'equipment') name = DN(getEquipTemplate(r.id));
                else if (r.type === 'pill') name = DN(getPill(r.id));
                else name = DN(getMaterial(r.id));
                return `<div style="display:flex;justify-content:space-between;padding:8px;border:1px solid rgba(201,169,106,0.2);border-radius:6px;margin-bottom:6px">
                    <span>${name}</span>
                    <span style="color:#d4af37">${r.sellPrice}${T('expl.stone','灵石')}</span>
                </div>`;
            }).join('');
            UI.showModal({
                title: T('expl.oldStall','老叟摊位'),
                body: `<p style="margin-bottom:10px">${T('expl.oldSmile','老叟神秘一笑，取出三件物品：')}</p>${body}<p>${T('expl.curStone','当前灵石：')}${fmtNum(player.stone)}</p>`,
                footer: random.map((r, i) => ({
                    text: T('expl.buy','买') + (i + 1),
                    action: () => {
                        if (player.stone >= r.sellPrice) {
                            player.stone -= r.sellPrice;
                            Inventory.addItem(player, r.id, 1);
                            if (typeof UI !== 'undefined') {
                                UI.toast(T('expl.buyOk','购买成功'), 'gold');
                                UI.hideModal();
                                UI.renderAll();
                            }
                            SaveSystem.save(player);
                ...
                })).concat([{ text: T('expl.leave','离开'), action: () => { if (typeof UI !== 'undefined') UI.hideModal(); } }])
            });
        }
    },
    ...
    giveRandomGongfa(player) {
        ...
        if (candidates.length === 0) {
            ...
            if (typeof UI !== 'undefined') UI.toast(T('expl.gain','获得') + fmtNum(xiu) + T('expl.zhanli','战力'), 'gold');
            return;
        }
...
const Quests = {
    ...
    claim(player, questId) {
        ...
        if (progress < q.target) {
            if (typeof UI !== 'undefined') UI.toast(T('expl.questNotDone','任务未完成'), 'bad');
            return;
        }
        state.claimed = true;
        ...
        if (typeof UI !== 'undefined') {
            UI.toast(T('expl.claimReward','领取奖励：') + (_gotStone ? fmtNum(_gotStone) + T('expl.stoneSpace','灵石 ') : '') + (_gotXiu ? fmtNum(_gotXiu) + T('expl.zhanli','战力') : ''), 'gold');
            UI.addLog(T('expl.completeLog','完成任务 ') + DN(q) + T('expl.claimReward2','，领取奖励'), 'evt');
            UI.renderAll();
        }
        SaveSystem.save(player);
    }
