...
(function() {
    ...
    function init() {
        // 加载提示轮换
        const tips = [
            T('main.tip1','凝神聚气，开启仙途...'),
            T('main.tip2','道法自然，清静无为...'),
            T('main.tip3','气沉丹田，意守玄关...'),
            T('main.tip4','紫气东来，万象更新...'),
            T('main.tip5','太上忘情，大道至简...')
        ];
        let tipIdx = 0;
    ...
    function showSaveSelect(returnToGame) {
        ...
        const avatars = [T('main.av.dao','道'), T('main.av.xian','仙'), T('main.av.mo','魔'), T('main.av.fo','佛')];
        wrap.innerHTML = slots.map(s => {
            if (s.meta) {
                const r = getRealm(s.meta.realmIdx);
                const elemName = T('main.elem.' + s.meta.element, (GameConfig.elements[s.meta.element] || {}).name || '');
                const av = avatars[s.meta.avatar || 0] || T('main.av.dao','道');
                return `<div class="ss-slot occupied${s.current ? ' current' : ''}" onclick="window.__chooseSlot(${s.id},true)">
                    ...
                    <div class="ss-slot-info">
                        ...
                        <div class="ss-slot-sub">${DN(r)}${cnNum(s.meta.realmLayer)}${T('main.layer','层')} · ${esc(elemName)}</div>
                        <div class="ss-slot-time">${fmtAgo(s.meta.lastSave)}${T('main.cultivated','修炼')}</div>
                    </div>
                    <button class="ss-slot-del" title="${T('main.delSave','删除存档')}" onclick="event.stopPropagation();window.__deleteSlot(${s.id},'${esc(s.meta.name)}')">×</button>
                </div>`;
            }
            return `<div class="ss-slot empty" onclick="window.__chooseSlot(${s.id},false)">
                <div class="ss-slot-avatar ss-empty-icon">＋</div>
                <div class="ss-slot-info">
                    <div class="ss-slot-name">${T('main.emptySlot','空位')}</div>
                    <div class="ss-slot-sub">${T('main.newXianyuan','开辟新的仙缘')}</div>
                </div>
            </div>`;
        }).join('');
    ...
    function chooseSlot(id, occupied) {
        if (occupied) {
            ...
            if (!p) { UI.toast(T('main.loadFail','读取该存档失败'), 'bad'); return; }
            ...
            if (inGame) {
                UI.renderAll();
                UI.toast(T('main.switchedTo','已切换至「') + p.name + T('main.daotu','」的道途'), 'good');
            } else {
                enterGame();
                if (Game.offlineReward && Game.offlineReward.zhanli > 0) {
                    setTimeout(() => UI.showOfflineReward(Game.offlineReward), 500);
    ...
    function deleteSlotUI(id, name) {
        UI.showModal({
            title: T('main.delSaveTitle','删除存档'),
            body: `<p style="color:#f43f5e">${T('main.confirmDel','确定删除「')}<b>${esc(name)}</b>${T('main.delSaveQ','」的存档吗？')}</p><p style="margin-top:8px">${T('main.opIrrev','此操作')}<b>${T('main.irrev','不可恢复')}</b>${T('main.dot','。')}</p>`,
            footer: [
                { text: T('main.confirmDelBtn','确认删除'), type: 'danger', action: () => {
                    ...
                    UI.hideModal();
                    showSaveSelect(false);
                    UI.toast(T('main.deleted','已删除该道途'), 'good');
                }},
                { text: T('main.cancel','取消'), action: () => UI.hideModal() }
            ]
        });
    }
    ...
    function fmtAgo(ts) {
        ...
        if (s < 60) return T('main.justNow','刚刚');
        const m = Math.floor(s / 60); if (m < 60) return m + T('main.minAgo','分钟前');
        const h = Math.floor(m / 60); if (h < 24) return h + T('main.hrAgo','小时前');
        const d = Math.floor(h / 24); if (d < 30) return d + T('main.dayAgo','天前');
        const mo = Math.floor(d / 30); if (mo < 12) return mo + T('main.moAgo','个月前');
        return Math.floor(mo / 12) + T('main.yrAgo','年前');
    }
    ...
    function bindCharacterCreate() {
        ...
        document.getElementById('createGameBtn').onclick = () => {
            const name = ccState.name.trim() || T('main.noName','无名修士');
            ...
            enterGame();
            UI.toast(T('main.welcome','欢迎踏入仙途，') + name + T('main.exclaim','！'), 'gold');
            UI.addLog(T('main.byName','你以') + name + T('main.stepXian','之名，踏入仙途'), 'evt');
            // 新道途：弹出仙途主线·序章（发奖 + 弹窗）
    ...
    function updatePreview() {
        const avatars = [T('main.av.dao','道'), T('main.av.xian','仙'), T('main.av.mo','魔'), T('main.av.fo','佛')];
        const elemNames = { metal: T('main.elem.metal','金灵根'), wood: T('main.elem.wood','木灵根'), water: T('main.elem.water','水灵根'), fire: T('main.elem.fire','火灵根'), earth: T('main.elem.earth','土灵根') };
        document.getElementById('previewAvatar').textContent = avatars[ccState.avatar];
        document.getElementById('previewName').textContent = ccState.name.trim() || T('main.noName','无名修士');
    ...
    function enterGame() {
        ...
        bindGameEvents();
        // 欢迎日志
        UI.addLog(T('main.loaded','游戏已加载'), 'evt');
        ...
        if (typeof SaveSystem !== 'undefined' && SaveSystem.storageMode() === 'none' && !window.__storageWarned) {
            ...
            UI.showModal({
                title: T('main.saveUnavail','⚠️ 存档功能不可用'),
                body: T('main.saveUnavailBody', '<p>当前浏览器<b>禁止了本地存储</b>，最常见原因是：你是用 <code>file://</code> 双击 <code>index.html</code> 打开的。</p><p style="margin-top:10px">这会导致所有进度<b>无法保存</b>，刷新或关闭后会丢失——也就是「按存档没用」。</p><p style="margin-top:12px">✅ <b>解决方法（任选其一）</b>：</p><p style="margin-top:6px">① 用 <code>http://</code> 方式打开：在游戏目录执行 <code>python -m http.server 8080</code>，再访问 <code>http://localhost:8080</code>。</p>'),
                footer: [{ text: T('main.gotIt','已知晓'), action: () => UI.hideModal() }]
            });
        }
    }
    ...
    function lifespanZero(player) {
        ...
        Game.stopLifespanDecay();
        UI.showModal({
            title: T('main.lifeEnd','寿元已尽'),
            body: `<p style="line-height:1.8">${esc(player.name)}${T('main.lifeEndBody1','道友闭关悟道，终至寿元耗尽，')}<b style="color:#d4af37">${T('main.yuhua','羽化归虚')}</b>${T('main.dot','。')}</p>
                   <p style="margin-top:10px;color:#9a8e7a;font-size:13px">${T('main.lifeEndBody2','红尘一梦，仙缘已了。此身道途就此归于轮回——可另择仙缘，再踏仙途。')}</p>`,
            footer: [{
                text: T('main.reincarnate','重入轮回'), type: 'primary', action: () => {
                    // 当前道途已尽，清除该槽位
    ...
    function bindGameEvents() {
        ...
        function doSave() {
            ...
            if (ok && st && st.mode === 'local') {
                UI.toast(T('main.saveOk','存档成功'), 'good');
            } else if (ok) {
                UI.toast(T('main.tempSave','已临时保存到会话（关闭标签页前有效），建议用 http:// 打开本游戏以永久保存'), 'warn');
            } else {
                UI.toast(T('main.saveFail','存档失败：浏览器禁止本地存储（多为用 file:// 双击打开所致）。请用 http:// 方式打开'), 'bad');
            }
        }
        ...
        document.querySelectorAll('.inv-tab').forEach(tab => {
            tab.onclick = () => {
                ...
                document.getElementById('invDetail').innerHTML = '<p class="inv-detail-empty">' + T('main.selItem','选中物品查看详情') + '</p>';
                UI.renderInventory();
            };
        });
        ...
        document.getElementById('resetBtn').onclick = () => {
            UI.showModal({
                title: T('main.resetTitle','⚠️ 重置游戏'),
                body: T('main.resetBody', '<p style="color:#f43f5e">此操作将<strong>永久删除</strong>你的所有游戏进度！</p><p style="margin-top:10px">确定要重置吗？</p>'),
                footer: [
                    { text: T('main.confirmReset','确认重置'), type: 'danger', action: () => {
                        Game.reset();
                        UI.hideModal();
                        document.getElementById('settings-screen').classList.add('hidden');
                        document.getElementById('game-main').classList.add('hidden');
        ...
        document.addEventListener('keydown', e => {
            ...
            if (e.key.toLowerCase() === 's' && !e.ctrlKey) {
                if (Game.save()) UI.toast(T('main.saveOk','存档成功'), 'good');
            }
        });
    }
...
/* 层数英文化：覆盖 cultivate.js 的 cnNum（main.js 最后加载，确保 cnNum 已定义）。
   英文模式返回阿拉伯数字（如 1/2），避免中文 一/二 泄漏。 */
(function () {
  if (typeof cnNum !== 'function') return;
  var _cnNumZH = cnNum;
  cnNum = function (n) {
    if (CUR_LANG === 'en') return '' + n;
    return _cnNumZH(n);
  };
})();
